...
<template>
	<el-upload :action="uploadOptions.action" :headers="header" :multiple="control.props.multiple"
		:limit="control.props.limit" :show-file-list="control.props.showFileList" :file-list="data.fileList"
		:accept="control.props.accept" :disabled="control.props.disabled" :on-remove="handleRemove"
		:on-success="handleUploadSuccess" :on-exceed="handleExceed" :before-upload="beforeUpload">
		<el-button type="primary" :disabled="control.props.disabled || control.props.limit <= data.fileList.length">
			<el-icon>
				<SvgIcon name="upload" />
			</el-icon>
