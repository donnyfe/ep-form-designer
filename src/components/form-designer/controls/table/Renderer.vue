...
<template>
	<el-table empty-text="请添加数据" :data="thisModel" :stripe="control.props.stripe" :border="control.props.border"
		:max-height="control.props.maxHeight" style="width: 100%;">
		<el-table-column :label="col.name" :key="index" v-for="(col, index) in control.props.columns">
			<template #default="scope">
				<el-input :disabled="control.props.disabled" v-model="scope.row[col.name]" placeholder></el-input>
			</template>
		</el-table-column>
		<el-table-column v-if="control.props.cloneable || control.props.deleteable" label="操作" fixed="right" width="155">
			<template #default="scope">
				<el-button v-if="control.props.cloneable" :disabled="control.props.disabled" @click="copyRow(scope)" text
					type="primary">复制</el-button>
				<el-button v-if="control.props.deleteable" :disabled="control.props.disabled" @click="delRow(scope)" text
