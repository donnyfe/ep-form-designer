...
<template>
    <el-upload class="avatar-uploader" list-type="picture-card" :action="uploadOptions.action" :headers="header"
        :multiple="control.props.multiple" :limit="control.props.limit" :show-file-list="control.props.showFileList"
        :file-list="data.fileList" :accept="control.props.accept" :disabled="control.props.disabled"
        :on-remove="handleRemove" :on-success="handleUploadSuccess" :on-exceed="handleExceed"
        :before-upload="beforeUpload" :on-preview="handlePictureCardPreview">
        <el-icon>
            <SvgIcon name="plus" v-if="control.props.limit > data.fileList.length" />
            <SvgIcon name="forbid" v-else />
        </el-icon>
