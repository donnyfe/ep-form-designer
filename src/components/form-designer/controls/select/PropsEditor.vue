...
<template>
	<el-form label-width="90px">
		...
		<el-form-item label-width="0">
			<draggable class="space-y-1 w-full" handle=".cursor-move" :list="control.props.options" item-key="index"
				tag="div">
				<template #item="{ element, index }">
					<div class="flex items-center border-b py-1">
						...
						<el-input class="flex-grow px-1" placeholder="选项值" v-model="element.value"></el-input>
						<el-input class="flex-grow px-1" v-if="control.props.showOptionLabel" placeholder="显示文本"
		...
		<el-form-item label="默认值">
			<el-select v-model="control.props.defaultValue" clearable placeholder="请输入默认值">
				<el-option v-for="item in control.props.options"
					:label="control.props.showOptionLabel ? item.label : item.value" :value="item.value"></el-option>
			</el-select>
		</el-form-item>
	</el-form>
</template>
