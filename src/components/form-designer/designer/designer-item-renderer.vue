...
<template>
	<draggable :list="controls" item-key="id" class="form-item-draggable" @change="onChange" :sort="true"
		:group="{ name: 'com', pull: true, put: true }">
		<template #item="{ element, index }">
			<div @click.stop="handleSelect(element)" class="form-item-wrap"
				:class="{ 'is-selected': data.activeControl == element }"
				:style="{ width: (element.props.width * 100) / formProps.cols + '%' }">
				<el-form-item v-if="element.type != 'tab'" :class="element.props.customClass" :prop="element.id" :label-width="element.props.showLabel
					? element.props.labelWidth || formProps.labelWidth : '0'"
