...
import DesignerControlsPanel from './designer-controls-panel.vue';
import DesignerPropsPanel from './designer-props-panel.vue';
import DesignerItemRenderer from './designer-item-renderer.vue';
import FormViewer from '../viewer/index.vue';
		...
		default: [
			...
			},
			{
				...
				controls: ['input', 'textarea', 'inputnumber', 'select', 'radio', 'checkbox',
					'rate', 'date', 'time', 'switch', 'slider', 'divider']
			},
			{
				name: '高级组件',
...
<template>
	<div class="elp-form-designer">

		<main class="designer-main">
			<DesignerControlsPanel :controlGroups="controlGroups" :formData="formData" @click="setControls" />

			<div class="designer-panel">
				<div class="form-toolbar">
					<div class="form-devices">
						<el-icon @click="data.device = device" class="icon" :class="{ 'is-selected': data.device == device }"
					...
					<div class="form-opts">
						...
						<el-button text type="primary" @click="previewFrom" :disabled="formData.controls.length == 0">
							<el-icon>
				...
				<div class="form-editor-panel" :class="[data.device]">
					<el-form ref="form" class="elform" :label-position="formData.props.labelPosition"
						:label-width="formData.props.labelWidth" :size="formData.props.size" :class="[formData.props.customClass]"
						:status-icon="false" :show-message="false">
						<DesignerItemRenderer :controls="formData.controls" :uploadOptions="uploadOptions"></DesignerItemRenderer>
					</el-form>
				</div>
			</div>

			<DesignerPropsPanel :data="data" :formData="formData" />
		</main>
	</div>
	...
	<el-dialog v-model="previewVisible" destroy-on-close center fullscreen title="表单预览">
		...
		<template #footer>
			<div class="form-preview-footer">
				...
				<el-button type="primary" @click="onValidate">验证表单</el-button>
