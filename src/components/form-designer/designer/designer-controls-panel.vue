	...
	<div class="designer-controls-panel">
		<el-scrollbar>
			<el-collapse v-model="activeNames">
				<el-collapse-item class="control-title" :title="group.name" :name="group.name"
					v-for="(group, index) in controlGroups" :key="index">
					<draggable class="control-group" :list="group.types" item-key="type" tag="ul" :clone="clone" :sort="false"
						:group="{ name: 'com', pull: 'clone', put: false }">
						<template #item="{ element }">
							<li class="control-label" @click="addControls(element)">
								<el-icon>
